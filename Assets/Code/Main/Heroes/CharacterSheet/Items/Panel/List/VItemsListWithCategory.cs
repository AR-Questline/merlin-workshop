...
namespace Awaken.TG.Main.Heroes.CharacterSheet.Items.Panel.List {
    [UsesPrefab("Items/List/" + nameof(VItemsListWithCategory))]
    public class VItemsListWithCategory : VBaseItemsListUI, IItemsListTitle {
        [Title("Title")]
        [SerializeField] GameObject titleParent;
        [SerializeField] TextMeshProUGUI title;

        protected override void ConfigureList() {
            SetTitleActive(!string.IsNullOrEmpty(title.text) && !Target.IsEmpty);
            base.ConfigureList();
        }
