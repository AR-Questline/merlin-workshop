...
public class MeleeWeaponTrail : MonoBehaviour
{
	...
	[InfoBox("...Sqr values are initialized on start, so edit them only in play mode")]
	[ShowInInspector]
	...
	void Update()
	{
		...
		bool noPoints = _points == null || _points.Count == 0;
		if (!_emit && noPoints && _autoDestruct)
		{
			Destroy(_trailObject);
			Destroy(gameObject);
		}
			...
			if (theDistanceSqr > _minVertexDistanceSqr)
			{
				...
				if (make)
				{
					...
					if (_points.Count >= 4)
					{
						...
						//Log.When(LogType.Important)?.Info(" smoothTipList.Count: " + smoothTipList.Count);
						...
						for (int n = 0; n < smoothTipList.Count; ++n)
						{
							...
							//else
							//{
							//	Awaken.Utility.Debugging.Log.When(Awaken.Utility.Debugging.LogType.Important)?.Error(idx + "/" + _smoothedPoints.Count);
							//}
						}
					}
#endif
