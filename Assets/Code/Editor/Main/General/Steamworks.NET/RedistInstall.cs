...
public class RedistInstall {
	...
	static void CopyFile(string path, string filename, bool bCheckDifference) {
		...
		if (!File.Exists(strSource)) {
			Log.Important?.Warning(string.Format("[Steamworks.NET] Could not copy {0} into the project root. {0} could not be found in '{1}'. Place {0} from the Steamworks SDK in the project root manually.", filename, Path.Combine(strCWD, path)));
			return;
		}
		...
		if (File.Exists(strDest)) {
			...
			Log.Important?.Info(string.Format("[Steamworks.NET] {0} in the project root differs from the Steamworks.NET redistributable. Updating.... Please relaunch Unity.", filename));
		}
		else {
			Log.Important?.Info(string.Format("[Steamworks.NET] {0} is not present in the project root. Copying...", filename));
		}
		...
		if (File.Exists(strDest)) {
			Log.Important?.Info(string.Format("[Steamworks.NET] Successfully copied {0} into the project root. Please relaunch Unity.", filename));
		}
		else {
			Log.Important?.Warning(string.Format("[Steamworks.NET] Could not copy {0} into the project root. File.Copy() Failed. Please copy {0} into the project root manually.", Path.Combine(path, filename)));
		}
	}
