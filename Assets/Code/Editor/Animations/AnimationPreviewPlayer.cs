...
/// </summary>
    ...
    public static class AnimationPreviewPlayer {
        private const BindingFlags PRIVATE_FIELD_BINDING_FLAGS =
            BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.GetField;

        private const BindingFlags PUBLIC_FIELD_BINDING_FLAGS =
            BindingFlags.Public | BindingFlags.Instance | BindingFlags.GetField;

        private const BindingFlags PUBLIC_PROPERTY_BINDING_FLAGS =
        ...
        static void Update() {
            if (!DebugOptions.AutoplayAnimationClipPreview.Active) {
                return;
            }
            ...
            if (shouldFindTimeControl) {
                ...
                var avatarPreview = animationClipEditorType.GetField("m_AvatarPreview", PRIVATE_FIELD_BINDING_FLAGS)
                    ?.GetValue(animationClipEditor);
                if (avatarPreview == null) return;

                var timeControl = avatarPreviewType.GetField("timeControl", PUBLIC_FIELD_BINDING_FLAGS)
