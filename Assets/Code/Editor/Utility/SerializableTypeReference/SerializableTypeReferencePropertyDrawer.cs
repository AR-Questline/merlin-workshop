...
namespace Awaken.TG.Editor.Utility.SerializableTypeReference {
    ...
    /// <summary>
    /// Custom property drawer for <see cref="SerializableTypeReference"/> properties.
    /// </summary>
    [CustomPropertyDrawer(typeof(Awaken.Utility.SerializableTypeReference.SerializableTypeReference))]
    [CustomPropertyDrawer(typeof(SerializableTypeConstraintAttribute), true)]
    public sealed class SerializableTypeReferencePropertyDrawer : PropertyDrawer {
        ...
        private static List<Type> GetFilteredTypes(SerializableTypeConstraintAttribute filter) {
            var types = new List<Type>();

            var excludedTypes = (ExcludedTypeCollectionGetter != null ? ExcludedTypeCollectionGetter() : null);

            Type baseType = filter switch {
                SerializableClassTypeExtendsAttribute extendsAttribute => extendsAttribute.BaseType,
                SerializableImplementsAttribute implementsAttribute => implementsAttribute.InterfaceType,
                _ => null,
            };

            if (baseType != null) {
                foreach (var type in TypeCache.GetTypesDerivedFrom(baseType)) {
                    FilterType(type, filter, excludedTypes, types);
                }
            } else {
        ...
        private static void FilterTypes(Assembly assembly, SerializableTypeConstraintAttribute filter,
            ICollection<Type> excludedTypes, List<Type> output) {
            foreach (var type in assembly.GetTypes()) {
                FilterType(type, filter, excludedTypes, output);
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void FilterType(Type type, SerializableTypeConstraintAttribute filter, ICollection<Type> excludedTypes,
            List<Type> output, bool duplicatesCheck = false) {
            if (!type.IsVisible || !(type.IsClass || type.IsValueType))
                return;
            ...
            if (duplicatesCheck && output.Contains(type))
                return;
        ...
        private static readonly int s_ControlHint = typeof(SerializableTypeReferencePropertyDrawer).GetHashCode();
        ...
        private static string DrawTypeSelectionControl(Rect position, GUIContent label, string classRef,
            SerializableTypeConstraintAttribute filter) {
            ...
            switch (Event.current.GetTypeForControl(controlID)) {
                ...
                case EventType.Repaint:
                    ...
                    if (s_TempContent.text == "") {
                        ...
                        var resolvedType = ResolveType(classRef);
                        if (resolvedType == null) {
                            s_TempContent.text += " {Missing}";
                        } else if (filter.ShowShortName) {
                            s_TempContent.text = resolvedType.Name;
                        }
                    }
            ...
            if (triggerDropDown) {
                ...
                DisplayDropDown(position, filteredTypes, ResolveType(classRef), filter?.Grouping ?? ClassGrouping.ByNamespace);
            }
        ...
        private static void OnSelectedTypeName(object userData) {
            ...
            s_SelectedClassRef = Awaken.Utility.SerializableTypeReference.SerializableTypeReference.GetClassRef(selectedType);
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            DrawTypeSelectionControl(position, property.FindPropertyRelative("_classRef"), label,
                    attribute as SerializableTypeConstraintAttribute);
        }
