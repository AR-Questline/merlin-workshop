...
namespace Awaken.TG.Editor.DataViews.Filters {
    [Serializable]
    public class DataViewEnumFilter<TEnum> : DataViewFilter where TEnum : Enum {
        [SerializeField] Operator @operator;
        [SerializeField] List<TEnum> operands = new();
        
        public override bool Match(DataViewValue value) {
            return @operator switch {
                Operator.IsOneOf => operands.Contains(value.intValue.ToEnum<TEnum>()),
                Operator.IsNot => !operands.Contains(value.intValue.ToEnum<TEnum>()),
                _ => false,
            };
        }
        ...
        public override void Draw(Rect rect, ref bool changed) {
            ...
            for (int i = 0; i < operands.Count; i++) {
                ...
                var newOperand = (TEnum)EditorGUI.EnumPopup(enumRects.AllocateWithRest(80), operand);
