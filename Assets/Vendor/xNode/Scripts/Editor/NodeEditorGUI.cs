...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow {
        ...
        void DrawAAPolyLineNonAlloc(float thickness, Vector2 p0, Vector2 p1, NodePort port) {
            ...
            // Check is bezier hovered by mouse, ignore if editing text field to improve text editor performance
            if (!EditorGUIUtility.editingTextField && HandleUtility.DistanceToLine(p0, p1) <= 5f) {
                hoveredConnection.port = port;
            }
        }

        /// <summary> Draw a bezier from output to input in grid coordinates </summary>
        public void DrawNoodle(Gradient gradient, NoodlePath path, NoodleStroke stroke, float thickness, List<Vector2> gridPoints, NodePort port = null, float noodleDensity = 1f) {
            // convert grid points to window points
            for (int i = 0; i < gridPoints.Count; ++i) {
                gridPoints[i] = GridToWindowPosition(gridPoints[i]) + new Vector2(0, -4f);
            }

            Color originalHandlesColor = Handles.color;
            Handles.color = gradient.Evaluate(0f);
            int length = gridPoints.Count;
            s_selected = selectedConnections.Any(c => c.port == port);
            ...
            switch (path) {
                case NoodlePath.Curvy:
                    ...
                    for (int i = 0; i < length - 1; i++) {
                        ...
                        // Hover effect.
                        int division = Mathf.RoundToInt(noodleDensity * .2f * dist_ab) + 3;
                        ...
                        for (int j = 1; j <= division; ++j) {
                            ...
                            DrawAAPolyLineNonAlloc(thickness, bezierPrevious, bezierNext, port);
                ...
                case NoodlePath.Straight:
                    for (int i = 0; i < length - 1; i++) {
                        ...
                        int segments = (int) (noodleDensity * Vector2.Distance(point_a, point_b) / 5);
                        ...
                        for (int j = 0; j <= segments; j++) {
                            ...
                            if (draw > 0) {
                                ...
                                DrawAAPolyLineNonAlloc(thickness, prev_point, lerp, port);
                            }
                            prev_point = lerp;
                ...
                case NoodlePath.Angled:
                    for (int i = 0; i < length - 1; i++) {
                        ...
                        if (gridPoints[i].x <= gridPoints[i + 1].x - (50 / zoom)) {
                            ...
                            end_1.x = midpoint;
                            if (i == length - 2) {
                                DrawAAPolyLineNonAlloc(thickness, gridPoints[i], start_1, port);
                                Handles.color = gradient.Evaluate(0.5f);
                                DrawAAPolyLineNonAlloc(thickness, start_1, end_1, port);
                                Handles.color = gradient.Evaluate(1f);
                                DrawAAPolyLineNonAlloc(thickness, end_1, gridPoints[i + 1], port);
                            } else {
                                DrawAAPolyLineNonAlloc(thickness, gridPoints[i], start_1, port);
                                DrawAAPolyLineNonAlloc(thickness, start_1, end_1, port);
                                DrawAAPolyLineNonAlloc(thickness, end_1, gridPoints[i + 1], port);
                            ...
                            if (i == length - 2) {
                                ...
                                DrawAAPolyLineNonAlloc(thickness, start_1, start_2, port);
                                Handles.color = gradient.Evaluate(0.5f);
                                DrawAAPolyLineNonAlloc(thickness, start_2, end_2, port);
                                Handles.color = gradient.Evaluate(0.75f);
                                DrawAAPolyLineNonAlloc(thickness, end_2, end_1, port);
                                Handles.color = gradient.Evaluate(1f);
                                DrawAAPolyLineNonAlloc(thickness, end_1, gridPoints[i + 1], port);
                            } else {
                                DrawAAPolyLineNonAlloc(thickness, gridPoints[i], start_1, port);
                ...
                case NoodlePath.ShaderLab:
                    ...
                    Handles.color = gradient.Evaluate(0f);
                    DrawAAPolyLineNonAlloc(thickness, start, gridPoints[0], port);
                    Handles.color = gradient.Evaluate(1f);
                    DrawAAPolyLineNonAlloc(thickness, end, gridPoints[length - 1], port);
                    for (int i = 0; i < length - 1; i++) {
                        Vector2 point_a = gridPoints[i];
                        Vector2 point_b = gridPoints[i + 1];
                        // Draws the line with the coloring.
        ...
        public void DrawConnections() {
            ...
            float noodleDensity = NodeEditorPreferences.GetSettings().noodleDensity;
                ...
                foreach (XNode.NodePort output in node.Outputs) {
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++) {
                        ...
                        if (!_portConnectionPoints.TryGetValue(input, out toRect)) {
                            toRect = new Rect(GridToWindowPositionNoClipped(input.node.position), new Vector2(16, 16));
                        }

                        List<Vector2> reroutePoints = output.EDITOR_GetReroutePoints(k);

                        gridPoints.Clear();
                        gridPoints.Add(fromRect.center);
                        gridPoints.AddRange(reroutePoints);
                        gridPoints.Add(toRect.center);
                        DrawNoodle(noodleGradient, noodlePath, noodleStroke, noodleThickness, gridPoints, output, noodleDensity);
        ...
        private void DrawNodes() {
            ...
            if (e.type != EventType.Layout && e.type != EventType.MouseDown) {
                hoveredNode = null;
                hoveredPort = null;
            }
            ...
            for (int n = 0; n < graph.nodes.Count; n++) {
                ...
                var width = nodeEditor.GetWidth();
                const float Labelwidth = 0.3333333f * 0.80f;
                const float FieldWidth = 2 * Labelwidth;
                GUIUtils.PushLabelWidth(Labelwidth * width);
                GUIUtils.PushFieldWidth(FieldWidth * width);
                GUIUtils.PushContextWidth(width);
                //Draw node contents
                ...
                if (e.type == EventType.Repaint) {
                    ...
                    foreach (var kvp in NodeEditor.portPositions) {
                        ...
                        Rect rect = new Rect(portHandlePos.x - 16, portHandlePos.y - 16, 32, 32);
        ...
        private bool ShouldBeCulled(XNode.Node node) {
            if (!nodeSizes.ContainsKey(node)) {
                foreach (NodePort input in node.Inputs) {
                    if (input != null && !_portConnectionPoints.ContainsKey(input)) {
                        return false;
                    }
                }
            }
