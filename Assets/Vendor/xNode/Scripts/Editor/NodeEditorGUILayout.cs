...
namespace XNodeEditor {
    ...
    public static class NodeEditorGUILayout {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            XNode.NodePort port = node.GetPort((NodePort.FieldNameCompressed)property.name);
        ...
        /// <summary> Make a simple port field. </summary>
        public static void PortField(GUIContent label, XNode.NodePort port, params GUILayoutOption[] options) => PortField(label, port, null, null, options);

        public static void PortField(GUIContent label, XNode.NodePort port, Color? backgroundColor = null, Color? color = null, params GUILayoutOption[] options) {
            if (port == null) return;
            if (options == null) options = new GUILayoutOption[] { GUILayout.MinWidth(30) };
            Vector2 position = Vector3.zero;
            GUIContent content = label != null ? label : new GUIContent(ObjectNames.NicifyVariableName(port.fieldNameCompressed));

            // If property is an input, display a regular property field and put a port handle on the left side
            if (port.direction == XNode.NodePort.IO.Input) {
                // Display a label
                EditorGUILayout.LabelField(content, NodeEditorResources.styles.inputPortLabel, options);
            ...
            PortField(position, port, backgroundColor, color);
        }

        /// <summary> Make a simple port field. </summary>
        public static void PortField(Vector2 position, XNode.NodePort port, Color? backgroundColor = null, Color? color = null) {
            if (port == null) return;

            Rect rect = new Rect(position, new Vector2(16, 16));

            NodeEditor editor = NodeEditor.GetEditor(port.node, NodeEditorWindow.current);
            backgroundColor ??= editor.GetTint();
            color ??= NodeEditorWindow.current.graphEditor.GetPortColor(port);
            DrawPortHandle(rect, backgroundColor.Value, color.Value, port.connectionType == Node.ConnectionType.Multiple);
        ...
        public static void DrawPortHandle(Rect rect, Color backgroundColor, Color typeColor, bool usePentagon = false) {
            // For some reason ports are positioned wrong in relation to noodles, so we needed to move one of them
            Color col = GUI.color;
            GUI.color = backgroundColor;
            GUI.DrawTexture(rect, usePentagon ? NodeEditorResources.pentagonOuter : NodeEditorResources.dotOuter);
            GUI.color = typeColor;
            GUI.DrawTexture(rect, usePentagon ? NodeEditorResources.pentagon : NodeEditorResources.dot);
            GUI.color = col;
        }
        ...
        private static ReorderableList CreateReorderableList(string fieldName, List<XNode.NodePort> dynamicPorts, SerializedProperty arrayData, Type type, SerializedObject serializedObject, XNode.NodePort.IO io, XNode.Node.ConnectionType connectionType, XNode.Node.TypeConstraint typeConstraint, Action<ReorderableList> onCreation) {
            ...
            list.drawElementCallback =
                (Rect rect, int index, bool isActive, bool isFocused) => {
                    XNode.NodePort port = node.GetPort((NodePort.FieldNameCompressed)(fieldName + " " + index));
                    ...
                    } else EditorGUI.LabelField(rect, port != null ? port.fieldNameCompressed : "");
                    if (port != null) {
                ...
                (ReorderableList rl) => {
                    ...
                    if (rl.index > reorderableListIndex) {
                        for (int i = reorderableListIndex; i < rl.index; ++i) {
                            XNode.NodePort port = node.GetPort((NodePort.FieldNameCompressed)(fieldName + " " + i));
                            XNode.NodePort nextPort = node.GetPort((NodePort.FieldNameCompressed)(fieldName + " " + (i + 1)));
            ...
            list.onAddCallback =
                (ReorderableList rl) => {
                    ...
                    int i = 0;
                    while (node.HasPort((NodePort.FieldNameCompressed)newName)) newName = fieldName + " " + (++i);
            ...
            list.onRemoveCallback =
                (ReorderableList rl) => {
                    ...
                    if (dynamicPorts[index] == null) {
                        ...
                        node.RemoveDynamicPort(dynamicPorts[dynamicPorts.Count() - 1].fieldNameCompressed);
                        serializedObject.Update();
                        EditorUtility.SetDirty(node);
                    }
            ...
            if (hasArrayData) {
                ...
                while (dynamicPortCount < arrayData.arraySize) {
                    ...
                    int i = 0;
                    while (node.HasPort((NodePort.FieldNameCompressed)newName)) newName = arrayData.name + " " + (++i);
