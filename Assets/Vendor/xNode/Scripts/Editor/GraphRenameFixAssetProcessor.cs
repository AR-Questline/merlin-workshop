...
namespace XNodeEditor {
    ...
    internal sealed class GraphRenameFixAssetProcessor : AssetPostprocessor {
        private static void OnPostprocessAllAssets(
            ...
            foreach (string asset in movedAssets.Union(importedAssets)) {
                Node nodeAsset = AssetDatabase.LoadMainAssetAtPath(asset) as Node;
                ...
                if (nodeAsset != null && AssetDatabase.IsMainAsset(nodeAsset)) {
                    AssetDatabase.SetMainObject(nodeAsset.graph, asset);
