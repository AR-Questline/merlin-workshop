...
namespace XNodeEditor {
    public partial class NodeEditorWindow {
        ...
        public NodeActivity currentActivity = NodeActivity.Idle;
        public bool isPanning { get; private set; }
        public Vector2[] dragOffset = Array.Empty<Vector2>();
        ...
        private bool IsHoveringConnection {get { return hoveredConnection.port != null;} }
        ...
        public void Controls() {
            ...
            switch (e.type) {
                        ...
                        if (IsHoveringPort) {
                            if (hoveredPort.IsOutput) {
                                ...
                                if (hoveredPort.IsConnected) {
                                    ...
                                    draggedOutputReroutes = output.EDITOR_GetReroutePoints(outputConnectionIndex);
                            ...
                            // If connection isn't selected
                            if (!selectedConnections.Contains(hoveredConnection)) {
                                // Add it
                                if (e.control || e.shift) selectedConnections.Add(hoveredConnection);
                                // Select it
                                else {
                            ...
                            // Deselect
                            else if (e.control || e.shift) selectedConnections.Remove(hoveredConnection);
                            e.Use();
                            currentActivity = NodeActivity.HoldNode;
                        }
                ...
                case EventType.MouseUp:
                    if (e.button == 0) {
                        ...
                        if (IsDraggingPort) {
                            ...
                            if (draggedOutputTarget != null && draggedOutput.CanConnectTo(draggedOutputTarget)) {
                                ...
                                if (connectionIndex != -1) {
                                    draggedOutput.EDITOR_GetReroutePoints(connectionIndex).AddRange(draggedOutputReroutes);
                                    if (NodeEditor.onUpdateNode != null) NodeEditor.onUpdateNode(node);
                            ...
                            // Open context menu for auto-connection if there is no target node
                                ...
                                INodeGenericMenu menu = new NodeGenericMenu();
                            ...
                            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
                        } else if (!IsHoveringNode && !IsHoveringInactiveArea()) {
                        ...
                        if (!isPanning) {
                            ...
                            } else if (IsHoveringReroute && !IsHoveringInactiveArea()) {
                                ShowRerouteContextMenu(hoveredReroute);
                            } else if (IsHoveringPort && !IsHoveringInactiveArea()) {
                                ShowPortContextMenu(hoveredPort);
                            } else if (IsHoveringNode && IsHoveringTitle(hoveredNode) && !IsHoveringInactiveArea()) {
                ...
                case EventType.KeyDown:
                    ...
                    else if (e.modifiers.HasFlag(EventModifiers.Control) && e.keyCode == KeyCode.F) Home();
                    if (NodeEditorUtilities.IsMac()) {
                        if (e.keyCode == KeyCode.Return) RenameSelectedNode();
                    } else {
                        if (e.keyCode == KeyCode.F2) RenameSelectedNode();
                    }
                    if (e.modifiers.HasFlag(EventModifiers.Control) && e.keyCode == KeyCode.A) {
        ...
        public void RemoveSelectedNodes() {
            ...
            selectedConnections.ForEach(c => {
                c.port.ClearConnections();
            });
            selectedConnections.Clear();
            foreach (UnityEngine.Object item in selectionCache) {
        ...
        private void InsertDuplicateNodes(XNode.Node[] nodes, Vector2 topLeft) {
            ...
            for (int i = 0; i < nodes.Length; i++) {
                ...
                NodeEditor.GetEditor(srcNode, current).Copy(srcNode, newNode);
                newNodes[i] = newNode;
                
                if (newNodes[i] is Node node) {
                    node.OnJustPasted();
                }
            }
                ...
                foreach (XNode.NodePort port in srcNode.Ports) {
                    for (int c = 0; c < port.ConnectionCount; c++) {
                        ...
                        if (substitutes.TryGetValue(inputPort.node, out newNodeIn) && substitutes.TryGetValue(outputPort.node, out newNodeOut)) {
                            ...
                            inputPort = newNodeIn.GetInputPort(inputPort.fieldNameCompressed);
                            outputPort = newNodeOut.GetOutputPort(outputPort.fieldNameCompressed);
                        }
        ...
        bool IsHoveringTitle(XNode.Node node) {
            ...
            HelpButtonAttribute helpButton = node.GetType().GetCustomAttribute<HelpButtonAttribute>();
            if (!string.IsNullOrWhiteSpace(helpButton?.url)) {
                width -= 35;
            }
            
            NodeEditor editor = NodeEditor.GetEditor(node, this);
            if (editor != null) {
                width -= editor.PostNameHeaderWidth;
                width -= editor.PreNameHeaderWidth;
                nodePos.x += editor.PreNameHeaderWidth;
            }
        ...
        public event Func<bool> IsHoveringInactiveAreaCustomFunc;
        bool IsHoveringInactiveArea() {
            return IsHoveringInactiveAreaCustomFunc?.Invoke() ?? false;
        }
    }
}
