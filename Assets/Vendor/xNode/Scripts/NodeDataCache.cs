...
namespace XNode {
    ...
    public static class NodeDataCache {
        ...
        public static void UpdatePorts(Node node, Dictionary<NodePort.FieldNameCompressed, NodePort> ports) {
            if (!Initialized) BuildCache();

            Dictionary<NodePort.FieldNameCompressed, NodePort> staticPorts = new Dictionary<NodePort.FieldNameCompressed, NodePort>();
            ...
            if (portDataCache.TryGetValue(nodeType, out typePortCache)) {
                for (int i = 0; i < typePortCache.Count; i++) {
                    staticPorts.Add(typePortCache[i].fieldNameCompressed, portDataCache[nodeType][i]);
                }
            }
            ...
            foreach (NodePort port in ports.Values.ToList()) {
                ...
                NodePort staticPort;
                if (staticPorts.TryGetValue(port.fieldNameCompressed, out staticPort)) {
                    ...
                    if (port.IsDynamic || port.direction != staticPort.direction || port.connectionType != staticPort.connectionType || port.typeConstraint != staticPort.typeConstraint) {
                        ...
                        if (!port.IsDynamic && port.direction == staticPort.direction) removedPorts.Add(port.fieldNameCompressed, port.GetConnections());
                        port.ClearConnections();
                        ports.Remove(port.fieldNameCompressed);
            ...
            foreach (NodePort staticPort in staticPorts.Values) {
                if (!ports.ContainsKey(staticPort.fieldNameCompressed)) {
                    ...
                    if (removedPorts.TryGetValue(staticPort.fieldNameCompressed, out reconnectConnections)) {
                    ...
                    ports.Add(staticPort.fieldNameCompressed, port);
                }
            }
            ...
            foreach (NodePort listPort in dynamicListPorts) {
                ...
                NodePort backingPort = staticPorts[(NodePort.FieldNameCompressed)backingPortName];
...
#if UNITY_EDITOR
            var nodeTypes = UnityEditor.TypeCache.GetTypesDerivedFrom<Node>();
#else
