...
namespace TAO.VertexAnimation.Editor
{
    public class LitGUI : ShaderGUI
    {
		...
		private void VertexAnimationGUI()
		{
			...
			{
				bool value = mat.IsKeywordEnabled("_USE_INTERPOLATION_ON");
				MaterialProperty useInterpolation = FindProperty("_USE_INTERPOLATION", properties);

				EditorGUI.BeginChangeCheck();

				value = EditorGUILayout.Toggle(MakeLabel(useInterpolation, "For smooth animations."), mat.IsKeywordEnabled("_USE_INTERPOLATION_ON"));
				//materialEditor.ShaderProperty(useInterpolation, MakeLabel(useInterpolation, "For smooth animations."));

				if (EditorGUI.EndChangeCheck())
				{
					SetKeyword("_USE_INTERPOLATION_ON", value);
				}
			}

			{
				bool value = mat.IsKeywordEnabled("_USE_NORMALA_ON");
				MaterialProperty useNormalA = FindProperty("_USE_NORMALA", properties);

				EditorGUI.BeginChangeCheck();

				value = EditorGUILayout.Toggle(MakeLabel(useNormalA, "Apply vertex normals saved in the alpha channel of the position map."), mat.IsKeywordEnabled("_USE_NORMALA_ON"));
				...
				if (EditorGUI.EndChangeCheck())
				{
					SetKeyword("_USE_NORMALA_ON", value);
				}
			}
			...
			MaterialProperty animationsOffsets = FindProperty("_AnimationsOffsets", properties);
			materialEditor.ShaderProperty(animationsOffsets, MakeLabel(animationsOffsets, "This will be auto filled by the animation system."));
