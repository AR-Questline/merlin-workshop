...
namespace Unity.Entities.Editor
{
    abstract class ComponentElementBase : BindableElement
    {
        ...
        protected PropertyElement CreateContent<TValue>(IComponentProperty property, ref TValue value)
        {
            ...
            if (value is IWithUnityObjectRef withUnityObjectRef) {
                var objectField = new ObjectField("Ref");
                objectField.objectType = withUnityObjectRef.Type;
                objectField.value = withUnityObjectRef.Object;
                foldout.contentContainer.Add(objectField);
            }
            ...
            if (value is IWithDebugText withDebugText)
            {
                var debugText = new Label(withDebugText.DebugText);
                foldout.contentContainer.Add(debugText);
            }
