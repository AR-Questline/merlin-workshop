            ...
            if (0 != (delegates.PresentFunctionBits & (1 << functionIndex)))
            {
                ...
                else
                {
                    ...
                    if (delegatePtr == IntPtr.Zero) {
                        //TODO AddUnmanagedSystemType is called from the generated code and re-initializing delegates list without disposing
                        Debug.LogError($"Delegate ptr for function {functionType} of system {systemState->DebugName} is null");
                        return;
                    }
