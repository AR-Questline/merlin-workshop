...
namespace Unity.Entities
{
    ...
    public unsafe struct EntityArchetype : IEquatable<EntityArchetype>
    {
        ...
        public NativeArray<ComponentType> GetComponentTypesWith(in ComponentTypeSet with, Allocator allocator)
        {
            return GetComponentTypesWith(with, (AllocatorManager.AllocatorHandle)allocator);
        }

        public NativeArray<ComponentType> GetComponentTypesWith(in ComponentTypeSet with, AllocatorManager.AllocatorHandle allocator)
        {
            var archetypeCount = Archetype->TypesCount - 1;
            var types = CollectionHelper.CreateNativeArray<ComponentType>(archetypeCount + with.Length, allocator);

            // NOTE: Entity is excluded (Entity is always the first type in the archetype)
            var index = 0;
            for (var i = 0; i < archetypeCount; ++i) {
                types[index++] = Archetype->Types[i + 1].ToComponentType();
            }

            for (var i = 0; i < with.Length; ++i) {
                types[index++] = with.GetComponentType(i);
            }
