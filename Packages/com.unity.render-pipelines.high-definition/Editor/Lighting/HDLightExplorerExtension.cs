    ...
    class HDLightExplorerExtension : DefaultLightingExplorerExtension
    {
        ...
        protected virtual LightingExplorerTableColumn[] GetHDLightColumns()
        {
            return new[]
            {
                ...
                new LightingExplorerTableColumn(LightingExplorerTableColumn.DataType.Checkbox, HDStyles.Volumetric, "m_Intensity", 95, (r, prop, dep) =>          // 25: Volumetric
                {
                    if (!TryGetAdditionalLightData(prop, out var lightData)) {
                        EditorGUI.LabelField(r, "--");
                        return;
                    }
                    bool useVolumetric = lightData.affectsVolumetric;
                    EditorGUI.BeginChangeCheck();
                    useVolumetric = EditorGUI.Toggle(r, useVolumetric);
                    if (EditorGUI.EndChangeCheck()) {
                        Undo.RecordObject(lightData, "Changed light volumetric enabled");
                        lightData.affectsVolumetric = useVolumetric;
                    }
                }, (lprop, rprop) => {
                    TryGetAdditionalLightData(lprop, out var lLightData);
                    TryGetAdditionalLightData(rprop, out var rLightData);
                    ...
                    return lLightData.affectsVolumetric.CompareTo(rLightData.affectsVolumetric);
                }, (target, source) => {
                    if (!TryGetAdditionalLightData(target, out var tLightData) || !TryGetAdditionalLightData(source, out var sLightData))
                        return;

                    Undo.RecordObject(tLightData, "Changed light volumetric enabled");
                    tLightData.affectsVolumetric = sLightData.affectsVolumetric;
                }),
            };
        }
