...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    class LightCookieManager
    {
...
#if UNITY_EDITOR
        Dictionary<int, List<Component>> m_cookieTexturesSources = new();        
#endif
...
#if UNITY_EDITOR
            foreach (var sourcesList in m_cookieTexturesSources.Values) {
                ListPool<Component>.Release(sourcesList);
            }
            m_cookieTexturesSources.Clear();
#endif
...
#if UNITY_EDITOR
                LogCookieAtlasOverflowDetails();
#endif
                m_NoMoreSpace = true;
            }
        }

        void LogCookieAtlasOverflowDetails() {
#if UNITY_EDITOR
            ListPool<GameObject>.Get(out var objectsToPing);
            try {
                System.Reflection.FieldInfo fieldInfo = typeof(PowerOfTwoTextureAtlas).GetField(
                    "m_RequestedTextures",
                    System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                var fieldValue = fieldInfo.GetValue(m_CookieAtlas);
                Dictionary<int, Vector2Int> atlasTextures = fieldValue as Dictionary<int, Vector2Int>;
                if (atlasTextures == null) {
                    Debug.LogError("Field \"m_RequestedTextures\" is not valid Dictionary<int, Vector2Int>");
                    return;
                }

                var cookieAtlasSize = m_CookieAtlas.AtlasTexture.referenceSize;
                var sb = new System.Text.StringBuilder(128);
                sb.Append($"Cookie atlas of size ").Append(cookieAtlasSize.x).Append('x').Append(cookieAtlasSize.y)
                    .Append(" could not fit all textures: \n");
                var atlasTexturesKeyValuePairs = atlasTextures.ToArray();
                System.Array.Sort(atlasTexturesKeyValuePairs,
                    (pairA, pairB) => pairB.Value.magnitude.CompareTo(pairA.Value.magnitude));
                foreach (var (textureId, textureSize) in atlasTexturesKeyValuePairs) {
                    objectsToPing.Clear();
                    string textureName;
                    Texture texture = null;
                    if (textureId > 0) {
                        textureName = UnityEditor.AssetDatabase.GetAssetPath(textureId);
                        if (string.IsNullOrEmpty(textureName)) {
                            textureName = "Two textures combined";
                        } else {
                            texture = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture>(textureName);
                        }
                    } else {
                        texture = (UnityEditor.EditorUtility.InstanceIDToObject(textureId) as Texture);
                        textureName = texture?.name;
                    }

                    sb.Append(textureName).Append(": ").Append(textureSize.x).Append('x').Append(textureSize.y)
                        .Append('\n');
                    if (texture != null && texture.dimension == TextureDimension.Cube) {
                        sb.Length--;
                        sb.Append(". Warning - texture is a cube texture, so in project it will be 2x smaller. \n");
                    }
                    if (m_cookieTexturesSources.TryGetValue(textureId, out var sources)) {
                        sb.Append("Texture sources: \n");
                        foreach (var textureSource in sources) {
                            if (textureSource.gameObject == null) {
                                continue;
                            }

                            objectsToPing.Add(textureSource.gameObject);
                            AppendPathInSceneHierarchy(sb, textureSource.gameObject);

                            sb.Append(" component: ").Append(textureSource.GetType().Name).Append('\n');
                        }
                    }
                    Debug.LogError(
                        $"Texture {textureName} {textureSize.x}x{textureSize.y} potential sources of cookie atlas overflow: ", texture);
                    for (int i = 0; i < objectsToPing.Count; i++) {
                        var gameObject = objectsToPing[i];
                        Debug.LogError($"{i}: {gameObject.name}", gameObject);
                    }
                }
                ...
                ListPool<GameObject>.Release(objectsToPing);
            }
            static void AppendPathInSceneHierarchy(System.Text.StringBuilder sb, GameObject obj) {
                sb.Append("    ");
                int insertIndex = sb.Length;
                sb.Append(ObjectName(obj));
                while (obj.transform.parent != null) {
                    obj = obj.transform.parent.gameObject;
                    string objName = ObjectName(obj);
                    sb.Insert(insertIndex, '/');
                    sb.Insert(insertIndex, objName);
                }
                sb.Insert(insertIndex, ':');
                sb.Insert(insertIndex, obj.scene.name);
            }
                
            static string ObjectName(GameObject obj) {
        ...
        public void DebugAddTextureSource(Texture texture, Component source) {
#if UNITY_EDITOR
            if (texture == null || source == null) {
                return;
            }

            var textureId = texture.GetHashCode();
            if (m_cookieTexturesSources.TryGetValue(textureId, out var sourcesList) == false) {
                ListPool<Component>.Get(out sourcesList);
                m_cookieTexturesSources.Add(textureId, sourcesList);
            }
            sourcesList.Add(source);
#endif
        }
        
        public void DebugAddTextureSource(Texture textureA, Texture textureB, Component sourceA, Component sourceB) {
#if UNITY_EDITOR
            if (textureA == null && textureB == null) {
                return;
            }
            //Taken from Texture2DAtlas.GetTextureID(Texture textureA, Texture textureB)
            var textureId = textureA.GetHashCode() + 23 * textureB.GetHashCode();
            if (m_cookieTexturesSources.TryGetValue(textureId, out var sourcesList) == false) {
                ListPool<Component>.Get(out sourcesList);
                m_cookieTexturesSources.Add(textureId, sourcesList);
            }
            sourcesList.Add(sourceA);
            sourcesList.Add(sourceB);
#endif
        }
        
        public void DebugAddTextureWithoutSource(Texture texture) {
#if UNITY_EDITOR
            if (texture == null) {
                return;
            }
