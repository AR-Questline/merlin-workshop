...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    public partial class HDRenderPipeline
    {
        ...
        internal void ReserveCookieAtlasTexture(HDAdditionalLightData hdLightData, Light light, LightType lightType)
        {
            ...
            var lightCookieManager = m_TextureCaches.lightCookieManager;
            switch (lightType)
            {
                case LightType.Directional:
                {
                    ...
                    {
                        lightCookieManager.ReserveSpace(hdLightData.surfaceTexture);
                        lightCookieManager.DebugAddTextureSource(hdLightData.surfaceTexture, hdLightData);
                    }
                    lightCookieManager.ReserveSpace(light?.cookie);
                    if (light != null)
                    {
                        lightCookieManager.DebugAddTextureSource(light.cookie, light);
                    }
                    break;
                }
                case LightType.Point:
                    ...
                    {
                        lightCookieManager.ReserveSpaceCube(light.cookie, hdLightData.IESPoint);
                        lightCookieManager.DebugAddTextureSource(light.cookie, hdLightData.IESPoint, light, hdLightData);
                    }
                    else if (light?.cookie != null)
                    {
                        lightCookieManager.ReserveSpaceCube(light.cookie);
                        lightCookieManager.DebugAddTextureSource(light.cookie, light);
                    }
                    else if (hdLightData.IESPoint != null)
                    {
                        lightCookieManager.ReserveSpaceCube(hdLightData.IESPoint);
                        lightCookieManager.DebugAddTextureSource(hdLightData.IESPoint, hdLightData);
                    }
                    break;
                case LightType.Spot:
                case LightType.Pyramid:
                case LightType.Box:
                    ...
                    {
                        lightCookieManager.ReserveSpace(light.cookie, hdLightData.IESSpot);
                        lightCookieManager.DebugAddTextureSource(light.cookie, hdLightData.IESPoint, light, hdLightData);
                    }
                    else if (light?.cookie != null)
                    {
                    ...
                    {
                        lightCookieManager.ReserveSpace(hdLightData.IESSpot);
                        lightCookieManager.DebugAddTextureSource(hdLightData.IESPoint, hdLightData);
                    }
                    // Projectors lights must always have a cookie texture.
                    else if (lightType != LightType.Spot) {
                        var whiteTexture = Texture2D.whiteTexture;
                        lightCookieManager.ReserveSpace(whiteTexture);
                        lightCookieManager.DebugAddTextureWithoutSource(whiteTexture);
                    }
                    break;
                case LightType.Rectangle:
                case LightType.Disc:
                    ...
                    {
                        lightCookieManager.ReserveSpace(hdLightData.areaLightCookie, hdLightData.IESSpot);
                        lightCookieManager.DebugAddTextureSource(hdLightData.areaLightCookie, hdLightData.IESSpot, hdLightData, hdLightData);
                    }
                    else if (hdLightData.IESSpot != null)
                    {
                        lightCookieManager.ReserveSpace(hdLightData.IESSpot);
                    ...
                    {
                        lightCookieManager.ReserveSpace(hdLightData.areaLightCookie);
                        lightCookieManager.DebugAddTextureSource(hdLightData.areaLightCookie, hdLightData);
                    }
                    break;
                case LightType.Tube:
