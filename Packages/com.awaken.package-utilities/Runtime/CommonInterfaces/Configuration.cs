...
namespace Awaken.Utility {
    ...
    public static class Configuration {
        ...
        public static void InitializeData() {
            ...
            try {
                if (File.Exists(ConfigFilePrimaryPath)) {
                    Debug.Log($"Loading config file from {ConfigFilePrimaryPath}");
                    primaryJson = JObject.Parse(File.ReadAllText(ConfigFilePrimaryPath));
                }
                if (File.Exists(ConfigFileSecondaryPath)) {
                    Debug.Log($"Loading config file from {ConfigFileSecondaryPath}");
                ...
                if ((primaryJson, secondaryJson) is (not null, not null)) {
                    ...
                    Debug.LogError($"Cannot load configuration file from {ConfigFilePrimaryPath} nor {ConfigFileSecondaryPath}");
                    return;
                }

                Debug.Log($"Configuration from json:\n{finalJson}");

                var serializedConfig = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, string>>(finalJson);
                if (serializedConfig == null) {
                    Debug.LogError($"Cannot deserialize configuration file:\n{finalJson}");
                } else {
                    foreach (var (key, value) in serializedConfig) {
                        ConfigData[key.ToLowerInvariant()] = value;
                    }
                }
        ...
        public static bool GetBool(string key, bool defaultValue = false) {
            if (ConfigData.TryGetValue(key.ToLowerInvariant(), out var value)) {
                return bool.TryParse(value, out var result) ? result : defaultValue;
            }
            return defaultValue;
        }

        public static bool GetBoolExact(string key, bool defaultValue = false) {
        ...
        public static int GetIntExact(string key, int defaultValue = 0) {
            if (ConfigData.TryGetValue(key, out var value)) {
                return int.TryParse(value, out var result) ? result : defaultValue;
            }
            return defaultValue;
        ...
        public static float GetFloatExact(string key, float defaultValue = 0) {
            if (ConfigData.TryGetValue(key, out var value)) {
                return float.TryParse(value, out var result) ? result : defaultValue;
            }
            return defaultValue;
        ...
        public static string GetString(string key, string defaultValue = "") {
            return ConfigData.GetValueOrDefault(key.ToLowerInvariant(), defaultValue);
        }
        ...
        public static T Get<T>(string key, T defaultValue = default) {
            if (ConfigData.TryGetValue(key.ToLowerInvariant(), out var value)) {
                try {
                    ...
                    Debug.LogError($"Failed to deserialize configuration value for key {key}");
                    Debug.LogException(e);
                }
            }
            return defaultValue;
